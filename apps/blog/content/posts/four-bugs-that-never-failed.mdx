...
Although — I wrote that line about accessibility before I checked it, and when I did, this page was scoring 98. It had no `<main>` landmark. Every other page on the site had one; the article about invisible bugs was the page you could not navigate to.

The accessibility sweep had passed it, every time, because `landmark-one-main` is a best-practice rule and the sweep was only asking about WCAG A/AA. **A rule you never ask for is a rule that cannot fail.** It asks for both now, and that immediately turned up one more: two `<nav>` landmarks both called "Social", on the page where you go to find out who I am.

So: five bugs that never failed. The fifth was in the sentence claiming there were four.

It's still not fast enough, either. About 114 KiB of JavaScript ships for pages that are mostly text and barely need any. That's the next one.
