...
export interface SeoInput {
  ...
  /**
   * Root-relative path to this page's social card. Defaults to the site's own.
   *
   * A post passes its own, so a shared link shows the ARTICLE's title rather than the
   * site's tagline — the difference between "someone shared fabiomenchicchi.com" and
   * "someone shared this piece". Generated by tools/og-image.mjs; a post without one
   * fails the build (tools/check-og.mjs), because a missing card is a broken image in
   * someone else's feed, where you will never see it.
   */
  image?: string;
}
...
export function seoMeta({
  ...
  /*
   * ABSOLUTE on purpose: a crawler reads the tag out of context and will not resolve a
   * relative path.
   *
   ...
   *
   * The PNGs are generated from the site's own tokens and fonts by tools/og-image.mjs.
   */
  const cardUrl = `${origin}${image}`;
